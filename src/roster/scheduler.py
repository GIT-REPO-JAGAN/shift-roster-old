...
def _compute_week_offs(
    ...
    Rules:
        "weekends"              → Sat & Sun
        "rolling6th7th"         → every 6th and 7th working day (rolling)
    ...
    # Rolling 6th & 7th: every employee gets 2 consecutive days off
    # after every 5 working days (true rolling week)
    if "rolling" in week_off_rule:
        ns = [int(x) for x in re.findall(r"\d+", week_off_rule)]
        if not ns:
            ns = [6, 7]
        cycle = max(ns)          # e.g. cycle of 7 → offs on day 6 & 7
        offs: set[date] = set()
        count = peer_index       # stagger per employee
        for d in all_dates:
            count += 1
...
def _build_rotation_schedule(
    shifts_per_slot: list[str],   # e.g. ["M","M","A","A","N","N"]
    rotation_weeks: int,          # 1 / 2 / 3 / 4
    start: date,
    end: date,
    slot_index: int,              # 0-based position of this employee in the group
) -> dict[date, str]:
    """
    ...
    rotation_weeks = 0 means static (no rotation — assign slot permanently).
    """
    ...
    if rotation_weeks == 0 or n_slots == 0:
        # Static: every day gets the same shift for this slot
    ...
    for d in all_dates:
        ...
        # Which rotation cycle is this employee currently on?
        # Offset the starting slot by slot_index rotations
...
class EmployeeSchedule:
    ...
    daily:     dict[date, str]
    week_offs: set[date]
    leaves:    set[date]        # PL dates
    comp_offs: set[date]        # CO dates
...
class ShiftScheduler:
    ...
    {
        "count": 6,
        "allocation": [2, 2, 2],        # employees per shift slot (M, A, N)
        "shifts": ["M", "A", "N"],      # shift codes matching allocation
        "rotation_weeks": 2,            # 0 = static, 1/2/3/4 = N-week rotation
        "week_off": "rolling6th7th",    # or "weekends", "every5th_6th", etc.
    }
    """
    ...
    def __init__(self, cfg: AppConfig, employees: list[Employee]) -> None:
        ...
        # Pre-group employees by skill to assign positional slot indices
    ...
    def _expand_slots(self, rule: dict) -> list[str]:
        ...
        e.g. shifts=["M","A","N"], allocation=[2,2,2]
             → ["M","M","A","A","N","N"]
        """
        ...
        if not allocation:
            # Fallback: equal weight for every shift
            return shifts
    ...
    def _schedule_employee(self, emp: Employee) -> EmployeeSchedule:
        ...
        # Build shift assignments per day from rotation engine
        ...
        # Build final daily schedule — priority: PL > CO > H > W > rotation
        daily: dict[date, str] = {}
