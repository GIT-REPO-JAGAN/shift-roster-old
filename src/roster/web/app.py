...
def _parse_rules_extended(rules_text: str) -> dict:
    ...
    for line in rules_text.splitlines():
        ...
        # Parse shift allocation: "2 in M, 2 in A, 2 in N" or "G" or "M / A / N"
        shifts:     list[str] = []
        allocation: list[int] = []

        alloc_upper = alloc_raw.upper()

        # Pattern: "2 in M, 2 in A, 2 in N"
        slot_matches = re.findall(r"(\d+)\s+in\s+([MANEG]+)", alloc_upper)
        if slot_matches:
            for cnt, shift in slot_matches:
                allocation.append(int(cnt))
                shifts.append(shift.strip())
        elif re.match(r"^G\s*(ONLY)?$", alloc_upper):
            shifts     = ["M", "A", "N"]
            allocation = []
