...
def _parse_leave_lines(raw: str, year: int, month: int) -> dict[str, list]:
    ...
    Supported formats (all case-insensitive):
        ...
        Per Day    – Name – Adhoc Shift: 05 | M
        Period Time – Name – Adhoc Shift: 06 to 09 | N
        Period Time – Name – Adhoc Shift: 06-09 | A
    ...
    for line in raw.splitlines():
        ...
        # Use maxsplit=1 so "Name – Adhoc Shift: 05 | G" splits correctly
        ...
        if re.match(r"adhoc\s*shift\s*[:\-]", remainder, re.I):
            ...
            # Shift part may be "G" or "G / M / A / N" — take the first token
            if "|" in body:
                date_part, shift_part = body.split("|", 1)
                # Take first slash-separated token as the actual code
            ...
            # Handle "06 to 09" or "06-09" date range
            ...
            if range_match:
                ...
                # Comma/space-separated individual days: "05, 08"
